import random
from DBUtills import update
from DBUtills import select
...
def bank_addUser(account, account_type, name, password, country, province, street, door, money):
    ...
    param2 = [account, account_type, name, password, country, province, street, door, money, "2021-11-22", bank_name]
    update(sql2, param2)
    return 1
...
def saveadd(name):
    if name in bank:
        ...
        sql3 = "insert into user set money(%s)"
        param3 = [bank[name]["money"]]
        update(sql3, param3)
        return True
    else:
        return False
...
def withdrawaladd(name, password):
    if name in bank and password == bank[name]["password"]:
        ...
        if money >= 0:
            bank[name]["money"] = money
            sql4 = "insert into user set money(%s)"
            param4 = [bank[name]["money"]]
            update(sql4, param4)
            return 0
        elif money < 0:
            return 3
